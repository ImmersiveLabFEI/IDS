...
function ALO = AntLionOptimizer(feat,label,opts)
% Parameters
lb    = 0;
ub    = 1; 
thres = 0.5; 
...
while t <= max_Iter 
  ...
  for i=1:N    
    ...
    rs = RouletteWheelSelection(prob);
    % Apply random walk of ant around antlion
    RA = RandomWalkALO(Xal(rs,:), c, d, max_Iter, dim);
    % Apply random walk of ant around elite
    RE = RandomWalkALO(Xe, c, d, max_Iter, dim);
...
%// Random Walk //
function RW = RandomWalkALO(Xal, c, d, max_Iter, dim)
% Pre
